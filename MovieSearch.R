...
# Read the Data
df <- read.table("http://www.utdallas.edu/~ond170030/data/MovieSummaries/plot_summaries_1500.txt", header = FALSE, sep="\t",stringsAsFactors = FALSE,quote="")
...
movie_function <- function(){
  movie_query <- "killer"

  while(movie_query != 'Q' || movie_query != 'q'){
    movie_query <- readline(prompt = "Press 'Q/q' to exit. Enter your Query :: ")
    movie_search(movie_query)
  }
